...
function Dialog({ gameState, updateGameState }) {
  ...
  // Safety check - if no NPC, create a default one or return to normal play
  useEffect(() => {
  ...
  // Safety check - if no NPC data, show error state
  if (!gameState.currentNPC) {
    return (
      <div className="dialog-container">
        <div className="error-message">
          No NPC found. Returning to exploration...
        </div>
      </div>
    );
  }
              ...
              gameState.character.inventory?.map((item, index) => (
                <div key={index} className="shop-item">
                  <div className="shop-item-info">
                    <div className="shop-item-name">{item.name}</div>
                    <div className="shop-item-description">{item.description}</div>
                    <div className="shop-item-price">Sell for: {Math.floor(item.price * 0.5)} gold</div>
                  </div>
                  <button 
